...
class VzaarGuzzle
{
    ...
    public static function whoAmI()
    {
        ...
        $req = VzaarGuzzle::setAuth($_url);

        $client = new GuzzleRequest($_url, HttpClientFactory::create());
        $client->verbose = VzaarGuzzle::$enableHttpVerbose;

        array_push($client->headers, $req->to_header());
        array_push($client->headers, 'User-Agent: Vzaar OAuth Client');
    ...
    public static function getAccountDetails($account)
    {
        ...
        $c = new GuzzleRequest(
            $_url . 'api/accounts/' . $account . '.json',
            HttpClientFactory::create()
        );
        $c->verbose = VzaarGuzzle::$enableHttpVerbose;
    ...
    public static function getUserDetails($account)
    {
        ...
        $req = new GuzzleRequest($_url . 'api/' . $account . '.json');
        $req->verbose = VzaarGuzzle::$enableHttpVerbose;
    ...
    public static function getVideoList(
        ...
        $req = new GuzzleRequest($_url, HttpClientFactory::create());
        $req->verbose = VzaarGuzzle::$enableHttpVerbose;

        if ($auth) array_push($req->headers, VzaarGuzzle::setAuth($_url, 'GET')->to_header());
     ...
     * @return VideoList
     */
    public static function searchVideoList(
        $username,
        $auth = false,
        $title = '',
        $labels = '',
    ...
    public static function getVideoDetails($id, $auth = false)
    {
        ...
        if ($auth) {
            array_push($req->headers, VzaarGuzzle::setAuth($_url, 'GET')->to_header());
        }
    ...
    {
        $signature = VzaarGuzzle::getUploadSignature();

        $client = new GuzzleRequest(
            $signature['vzaar-api']['upload_hostname'],
            HttpClientFactory::create()
        );
        $client->verbose = VzaarGuzzle::$enableHttpVerbose;

        $client->method = 'POST';
        $client->uploadMode = true;
        $client->useSsl = true;

        array_push($client->headers, 'User-Agent: Vzaar API Client');
        array_push($client->headers, 'x-amz-acl: ' . $signature['vzaar-api']['acl']);
        array_push($client->headers, 'Enclosure-Type: multipart/form-data');

        $s3Headers = [
            'AWSAccessKeyId' => $signature['vzaar-api']['accesskeyid'],
        ...
        $reply = $client->send($s3Headers, $path);

        $xmlObj = new XMLToArray($reply, [], [], true, false);
    ...
    public static function uploadSubtitle($language, $videoId, $body)
    {
        ...
        $req = VzaarGuzzle::setAuth($_url, 'POST');
        ...
        $c = new GuzzleRequest($_url, HttpClientFactory::create());
        $c->verbose = VzaarGuzzle::$enableHttpVerbose;
        $c->method = 'POST';
     ...
     */
    public static function uploadLink(
        $url,
        $title = NULL,
        $description = NULL,
    ...
    {
        $_url = "https://vzaar.com/api/v1.1/videos/signature";
        $_query = [];
        ...
        $req = VzaarGuzzle::setAuth($_url, 'GET');
        $req->verbose = false;

        $req_header = $req->to_header();

        $client = HttpClientFactory::create();

        $request = $client->createRequest('GET', $_url);
        $auth_params = explode("Authorization: ", $req_header)[1];
        $request->setHeader('Authorization', $auth_params);
        $request->setHeader('User-Agent', 'Vzaar OAuth Client');
        ...
        return UploadSignature::fromXml($response->getBody()->getContents());
    }
    ...
    public static function getUploadSignatureAsXml($redirectUrl = null, $multipart = false)
    {
        ...
        if (VzaarGuzzle::$enableFlashSupport) {
    ...
    public static function editVideo($id, $title, $description, $private = 'false', $seoUrl = '')
    {
        ...
        $req = VzaarGuzzle::setAuth($_url, 'PUT');

        $data = '<?xml version="1.0" encoding="UTF-8"?><vzaar-api><_method>post</_method><video><title>' .
            self::_sanitize_str($title) .
            '</title><description>' .
            self::_sanitize_str($description) .
            '</description>';
     ...
     * @return mixed
     */
    public static function processVideoCustomized(
        $guid,
        $title,
        $description,
        $labels,
    ...
    public static function setAuth($_url, $_method = 'GET')
    {
        ...
        $token = new OAuthToken(self::$config['secret'], self::$config['token']);
        $req = OAuthRequest::from_consumer_and_token($consumer, $token, $_method, $_url);
        $req->set_parameter('oauth_signature_method', 'HMAC-SHA1');
        $req->set_parameter(
            'oauth_signature',
            $req->build_signature(new OAuthSignatureMethod_HMAC_SHA1, $consumer, $token)
        );
